...
app.delete("/deleteProfile",upload.none(),async(req,res)=>{
 ...
 if(delResult.deletedCount > 0){
  res.json({Status:"Success",msg:"Account deleted Sucessfully"}) 
 }else{
...
let student = new mongoose.model("students",studentSchema,"2507MERNStudents");


let ConnectedToMDB = async()=>{
    try{
     await mongoose.connect("mongodb+srv://vemulajyothi24_db_user:mern2507@cluster0.ykdyn60.mongodb.net/Batch2507?appName=Cluster0");
     console.log("Successfully conneted to MDB");
   
    }catch(err){
     console.log("Umable to conneted  MDB")
    }
